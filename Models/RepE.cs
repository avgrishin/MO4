...
namespace MO.Models
{
  public class RepE
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}E.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение доходности, опережающей доходность индекса энергетической отрасли ММВБ.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Инвестирование в акции компаний энергетического сектора, обладающие наибольшим потенциалом роста на горизонте 12 месяцев.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      c = new PdfPCell(new Phrase(100f, "Помимо фундаментальных соображений, управляющий также учитывает промежуточные колебания спроса и предложения, резервируя часть активов в денежных средствах при ухудшении конъюнктуры рынка.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Энергетическая перспектива", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("ММВБ - Электроэнергетика", arial10)) { Border = 0 });
