...
namespace MO.Models
{
  public class RepER
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}ER.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Долгосрочный прирост капитала с целевой доходностью свыше 10% годовых, при максимальном отрицательном отклонении кривой доходности фонда не более 10%. Ожидание положительных результатов не должно превысить 3 месяца.", arial9)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Управляющий распределяет денежные средства фонда среди классов активов в следующей пропорции 50% на облигации, 30% long/short акции, 20% на деривативы. Для достижения поставленной доходности управляющий использует следующие стратегии:\n·	Directional/Opportunistic — покупка недооцененных активов и продажа переоцененных, возможность предугадать тот рынок, на котором произойдет сильное движение.\n·	Event-Driven — стратегия с корпоративным фокусом, основанная на ожидании важных событий (слияния/поглощения, выкуп акций, реструктуризация, спасение от банкротства);\n·	Relative value strategies  — совмещение длинных и коротких позиций по любым инструментам;\n·	Fixed Income Arbitrage;\n·	Equity Long-Short;\n·	Volatility trading;\n·	Global Macro.", arial9)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.1f);
      table.AddCell(c);
      table.AddCell(new PdfPCell(new Paragraph("Факторы риска", arial12b)) { Border = 2, BorderWidth = 0.8f, PaddingTop = 5f, PaddingBottom = 5f });
      c = new PdfPCell(new Phrase(100f, "Приоритетом инвестиционной политики должен стать прирост капитала с непрерывным контролем рисков. Риск инвестиционного Фонда может быть оценен, как средний. Фонд определяется, как Фонд диверсифицированных активов, которые котируются на организованных рынках, и управляются различными, в том числе разнонаправленными стратегиями.", arial9)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.1f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("Портфель Extra Return URALSIB DIVERSITY FUND", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
      ...
      t.AddCell(new PdfPCell(new Phrase("10% годовых", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
      table.AddCell(new PdfPCell(t) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("1,5 % СЧА", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
      t.AddCell(new PdfPCell(new Phrase("Вознаграждение за успех", arial9)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("20%", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
      table.AddCell(new PdfPCell(t) { Border = 0 });

      t = new PdfPTable(new float[] { 3.6f, 5.0f });
      t.AddCell(new PdfPCell(new Phrase("Минимальная сумма инвестирования", arial9)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("первоначальная – 100 000 USD, последующая 1 000 USD.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
      table.AddCell(new PdfPCell(t) { Border = 0 });
      mtable.AddCell(new PdfPCell(table) { Border = 0 });

      table = new PdfPTable(1);
      table.AddCell(new PdfPCell(new Paragraph("Крупнейшие вложения, % от СЧА", arial12b)) { Border = 2, BorderWidth = 0.8f, PaddingTop = 5f, PaddingBottom = 5f });
      //par1 = new Paragraph();
    ...
    private Stream getChart(int id)
    {
      ...
      else if (id == 22)
      {
        ...
        {
          Width = new System.Web.UI.WebControls.Unit(400, System.Web.UI.WebControls.UnitType.Pixel),
          Height = new System.Web.UI.WebControls.Unit(280, System.Web.UI.WebControls.UnitType.Pixel),
          AntiAliasing = AntiAliasingStyles.All,
          TextAntiAliasingQuality = TextAntiAliasingQuality.SystemDefault
        };
