...
namespace MO.Models
{
  public class RepF1
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}F1.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение доходности, превышающей доходность индекса РТС при сопоставимом уровне риска в долгосрочной перспективе.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Выбор наиболее недооцененных акций по фундаментальным показателям. Управляющий также может учитывать промежуточные колебания спроса и предложения, резервируя часть активов в денежных средствах при ухудшении конъюнктуры рынка. Средства фонда вкладываются преимущественно в «голубые фишки» фондового рынка – акции наиболее надежных и устойчивых российских предприятий.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Фонд Первый", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("Индекс РТС", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("14.11.1998", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
