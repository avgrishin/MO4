...
namespace MO.Models
{
  public class RepF2
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}F2.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение устойчивого дохода от вложений в российские акции за счёт эффективного выбора наиболее перспективных отраслей.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Выбор 2-3 отраслей с наиболее высоким потенциалом роста на 6-9 месяцев. При оценке потенциала учитываются как фундаментальные показатели, так и колебания спроса и предложения по конкретным акциям. В состав портфеля могут входить как «голубые фишки», так и акции «второго эшелона», если они относятся к перспективным.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Фонд Отраслевых инвестиций", arial10)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("Тип", arial10)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("Интервальный паевой инвестиционный фонд акций", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("04.03.1999", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
