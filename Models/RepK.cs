...
namespace MO.Models
{
  public class RepK
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}K.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Прирост стоимости активов в краткосрочной перспективе, превышающей ставку по годовым депозитам в первоклассных банках, за счёт формирования диверсифицированного портфеля рублёвых облигаций.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Отбор финансово устойчивых эмитентов, способных исполнить свои обязательства по облигациям с вероятностью, близкой к 100%. Регулирование вложений в зависимости от ожидаемой динамики процентных ставок на рынке: упор на краткосрочные облигации при росте ставок и долгосрочные – при снижении. Широкая диверсификация вложений по эмитентам и отдельным выпускам облигаций для обеспечения высокой ликвидности портфеля и минимизации кредитных рисков.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Фонд Консервативный", arial10)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("Тип", arial10)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("Открытый паевой инвестиционный фонд облигаций", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("RUX-Cbonds", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("1,5", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
      ...
      t.AddCell(new PdfPCell(new Phrase("0,5 - 1", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
