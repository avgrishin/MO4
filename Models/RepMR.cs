...
namespace MO.Models
{
  public class RepMR
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}MR.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение доходности, превышающей средние показатели акций металлургического сектора России.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Активы Фонда инвестируются в акции российских предприятий металлургии. При отборе конкретных эмитентов учитываются как фундаментальный потенциал роста, так и ожидаемые изменения спроса и предложения в краткосрочной перспективе.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Металлы России", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("ММВБ – Металлы и добыча", arial10)) { Border = 0 });
