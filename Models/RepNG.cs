...
namespace MO.Models
{
  public class RepNG
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}NG.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение доходности, превышающей средние показатели российских акций нефтегазового сектора.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Активы Фонда инвестируются в акции нефтегазового сектора России. При отборе конкретных эмитентов учитываются как фундаментальный потенциал роста, так и ожидаемые изменения спроса и предложения в краткосрочной перспективе.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Нефть и Газ", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("ММВБ – Нефть и Газ", arial10)) { Border = 0 });
