...
namespace MO.Models
{
  public class RepRF
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}RF.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение доходности, превышающей средние показатели акций финансового  сектора России.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Долгосрочное удержание позиций в акциях, имеющих наибольший фундаментальный потенциал доходности.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      c = new PdfPCell(new Phrase(100f, "Активы Фонда  инвестируются в акции банков, страховых компаний и других предприятий, максимально близких по сути бизнеса к финансовому сектору.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Российские финансы", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("РТС – Банки и Финансы", arial10)) { Border = 0 });
