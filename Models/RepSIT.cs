...
namespace MO.Models
{
  public class RepSIT
  {
    ...
    public Stream GetReport()
    {
      ...
      doc.Open();
      Image im = Image.GetInstance(string.Format(@"{0}SIT.png", ImagePath));
      ...
      var c = new PdfPCell(new Paragraph("Получение максимальной прибыли от реализации долгосрочного потенциала сектора информационных технологий и телекоммуникаций в России и стран бывшего СССР.", arial10)) { HorizontalAlignment = Element.ALIGN_JUSTIFIED, Border = 0 };
      ...
      c = new PdfPCell(new Phrase(100f, "Средства размещаются в акции операторов связи, производителей телекоммуникационного и вычислительного оборудования, провайдеров информационных услуг  в России и странах ближнего зарубежья.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      c = new PdfPCell(new Phrase(100f, "Портфель фонда ориентирован на долгосрочное удержание позиций в акциях, имеющих наибольший фундаментальный потенциал доходности.", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_JUSTIFIED };
      c.SetLeading(0f, 1.5f);
      table.AddCell(c);
      ...
      t.AddCell(new PdfPCell(new Phrase("УралСиб Связь и Информационные технологии", arial10)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("Тип", arial10)) { Border = 0 });
      t.AddCell(new PdfPCell(new Phrase("Открытый паевой инвестиционный фонд акций", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("ММВБ - Телекоммуникации", arial10)) { Border = 0 });
      ...
      t.AddCell(new PdfPCell(new Phrase("02.03.2008", arial10)) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT });
