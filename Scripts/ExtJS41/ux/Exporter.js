...
Ext.define("Ext.ux.exporter.excelFormatter.Worksheet", {
  ...
  buildHeader: function () {
    ...
    Ext.each(this.grid.columns, function (col) {
      var title;
      if (col.dataIndex && col.hidden != true && col.isExport != false) {
  ...
  buildRow: function (record, index) {
    ...
    Ext.each(this.grid.columns, function (col) {
      ...
      if (name && col.hidden != true && col.isExport != false) {
        //if given a renderer via a ColumnModel, use it and ensure data type is set to String
        if (Ext.isFunction(col.renderer) && col.xtype != "numbercolumn" && col.xtype != "checkcolumn" && record.fields.get(name).type.type != "date") {
          var value = col.renderer(record.get(name), null, record),
              type = record.fields.get(name).type.type == "date" ? "DateTime" : "String";
        } else {
          var value = record.get(name),
              type = this.typeMappings[col.xtype || col.type || record.fields.get(name).type.type];
        }
